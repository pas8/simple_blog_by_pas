...
const Profile: FC<{ posts: PostType[]; profileUser: ProfileDocType & { id: string }; isKicked: boolean }> = ({
  posts,
  profileUser,
  isKicked
}) => {
  ...
  const [isCensureDialogOpen, setIsCensureDialogOpen] = useState(isKicked);
  ...
  return (
    <>
      {isCensureDialogOpen ? (
        <Dialog
          plusZIndex={4}
          isOpen={isCensureDialogOpen}
          isIsolated
          title={'This user was kicked'}
          contentChildren={
            <>
              <Text style={{ maxWidth: 320, marginRight: 10 }}>
                We are not responsible for the fact that the content contained on this page can break your psyche, by
                pressing the button you fully agree with it
              </Text>
              <Button onClick={() => push(`/kicked/${profileUser.id}`)}>
          ...
          utilsChildren={
            <>
              ...
              {
                //@ts-ignore
                <Button isDangerous onClick={() => setIsCensureDialogOpen(false)}>
                  I understut, show page
                </Button>
              }
            </>
          }
              ...
              <>
                {defenderDialogState.contentChildrenName === 'kick' && (
                  <DialogContentContainer>
                    ...
                    />
                    <Text>Min symbols number of reason message is 42</Text>
                  </DialogContentContainer>
                )}
              </>
            }
...
export const getServerSideProps = async (context: any) => {
  ...
  const isKicked = kickedUserSnap.exists();
